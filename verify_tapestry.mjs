...
async function runVerification() {
    ...
    try {
        ...
        let patchedSwayJS = originalSwayJS.replace("import Logger from '@/js/core/Logger.js';", "const Logger = global.Logger;");
        await fs.writeFile(swayPath, patchedSwayJS);

        originalSwayDataJS = await fs.readFile(swayDataPath, 'utf8');
        let patchedSwayDataJS = originalSwayDataJS.replace("import Logger from '@/js/core/Logger.js';", "const Logger = global.Logger;");
