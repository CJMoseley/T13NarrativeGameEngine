...
window.T13NE.Suits.writeSuit = function (props){
	
	var suit = window.T13NE.Suits.getSuitNumber(props["data-suit"]);
	var mode= props["data-mode"];
	
	var qs =window.T13NE.Suits[suit];
	var qe=[];
		for (var index in qs){
			...
			////console.log("value", value);
				...
				if ( typeof value === 'string' || value instanceof String){
					qe[index] = window.T13NE.CE(window.T13NE.ContentBox, {"className":props.className+"-suit-"+cass, "title":titl, "data-description":value},null);
				}else{
					if (typeof value ==='object'){
						...
						for (var indexa in value){
							...
							qe[index] = window.T13NE.CE(window.T13NE.ContentBox,{"className":props.className+"-suit-"+cass, "title":titl, "data-type":indexa.replace("_"," "), "data-description":value[indexa]},null);
						}
					}else{
			...
			switch (mode){
				case "cardcorner":
					ret= (window.T13NE.CE("section", {"className":props.className+"-section t13ne-tooltip"},
						[window.T13NE.CE("span", {"className":props.className+"-span "+qs['className']},props.children),
						window.T13NE.CE("span", {"className":props.className+"-span "+qs['className'], dangerouslySetInnerHTML:{__html:qs['Html']}}),
						window.T13NE.CE("aside", {"className":props.className+"-aside t13ne-tooltip"},
							[qe['Description'],
							qe['Tarot_Text'],
							qe['Ordeal_Trump'],
							qe['Ordeal_Text'],
							qe['Parry'],
							qe['Soak'] ])]));
							//console.log("cardcorner=",ret);
							return ret;
				break;
				case undefined:
				default:
				
				ret= (window.T13NE.CE("details",{"className":props.className+"-details"}, 
					window.T13NE.CE("summary", {"className":props.className+"-summary "+qs["className"]}, qs['Icon']),
					qe['Description'],
					qe['Tarot_Text'],
					qe['Ordeal_Trump'],
					qe['Ordeal_Text'],
	...
	render(){
		...
		for (var index in qs){
			...
			////console.log("value", value);
				...
				if ( typeof value === 'string' || value instanceof String){
					qe[index] = window.T13NE.CE(window.T13NE.ContentBox, {"className":this.props.className+"-suit-"+cass, "title":titl, "data-description":value},null);
				}else{
					if (typeof value ==='object'){
						////console.log("obj value", value);
						for (var indexa in value){
							////console.log("indexa",indexa);
							////console.log("value[indexa]", value[indexa]);
							qe[index] = window.T13NE.CE(window.T13NE.ContentBox,{"className":this.props.className+"-suit-"+cass, "title":titl, "data-type":indexa.replace("_"," "), "data-description":value[indexa]},null);
						}
					}else{
						////console.log("type of value thing:", typeof value);
					}
