...
window.T13NE.FacetAspectBox = function (props){
	...
	if (face!==null&&window.T13NE.Facets[face]!==undefined){
		...
		window.T13NE.CE("main",{className:props.className+"-section-main t13ne-textbox-description"},
			...
			window.T13NE.Suits.writeSuit({"data-suit":facet['Suit'], "className":css+"-suit t13ne-facet-suit"})])]);
				...
				break;
				case "Suit":
					content=window.T13NE.Suits.writeSuit({"data-suit":facet['Suit'], "className":css+"-suit t13ne-facet-suit", "key":window.T13NE.KeyGen(title+"Suit") });
				break;
				case "FacetInitial":
				...
				break;
				case "Hitch":
				content=window.T13NE.CE("div",{className:css+'-div'},[window.T13NE.CE(window.T13NE.TextBox,{className:css+"-text", title:title, "data-type":{internal:facet["Hitch"]}, "data-description":{internal: facet["Hitch_Text"]}, "key":window.T13NE.KeyGen(facet[aspect])}, 
...
window.T13NE.FacetDisplay = function (props){
	...
	if (props["data-facets"]||props["data-facet"]){
		...
		facets.forEach( function(element, index) {
			...
			switch(mode){
			case "initial-only-list": ret =window.T13NE.CE("li",{"className":props.className+"-li", "key":window.T13NE.KeyGen(facet) + Math.random().toString(36).substr(2, 5)}, window.T13NE.FacetAspectBox({"className":props.className,"data-aspect":"FacetInitial", "data-facet":facet, "key":window.T13NE.KeyGen(13) }));
			break;
			case "initial-only": ret =window.T13NE.CE("span",{"className":props.className+"-span", "key":window.T13NE.KeyGen(facet) + Math.random().toString(36).substr(2, 5)}, window.T13NE.FacetAspectBox( {"className":props.className,"data-aspect":"FacetInitial", "data-facet":facet, "key":window.T13NE.KeyGen(13) }));
			break;
			case "name-only-list": ret = window.T13NE.CE("li",{"className":props.className+"-li", "key":window.T13NE.KeyGen(facet) + Math.random().toString(36).substr(2, 5)},window.T13NE.FacetAspectBox( {"className":props.className, "data-aspect": "FacetName", "data-facet":facet,"key":window.T13NE.KeyGen(13) }));
			break;
			case "name-only": ret = window.T13NE.CE("span",{"className":props.className+"-span", "key":window.T13NE.KeyGen(facet) + Math.random().toString(36).substr(2, 5)},window.window.T13NE.FacetAspectBox({"className":props.className, "data-aspect": "FacetName", "data-facet":facet,"key":window.T13NE.KeyGen(13) }));
			break;
			case "short":
			var asp=window.T13NE.Facets[facet];
				...
				//console.log("building for key",key);
						...
						return (window.T13NE.FacetAspectBox({"className":props.className+"-facet-aspect t13ne-facet-aspect", "data-facet":facet, "data-aspect":key, "key":window.T13NE.KeyGen('aspectbox'+facet) }));
					}else{
						return null;
					}
			...
			break;
			case "ordeal":
				ret.push(window.T13NE.FacetAspectBox({className:props.className+"-ordeal", "key":window.T13NE.KeyGen("ordeal"), "data-facet":facet, "data-aspect":"Test" }));
			break;
			case "card":
			default:
				var asp=window.T13NE.Facets[facet];
				...
				//console.log("cardkey=",key);
					...
					if (key.indexOf("_Text")==-1 && key!=="FacetIndex" && key!=="FacetInitial" && key!=="Suit" && key!=="Yang" &&key!=="Term" && key!=="FacetAdjectives"){
				//console.log("building for key",key);
						return (window.T13NE.CE("li",{"className":props.className+"-li t13ne-"+css, "key":window.T13NE.KeyGen("card") },window.window.T13NE.FacetAspectBox({"className":props.className+"-facet-aspect t13ne-facet-aspect", "data-facet":facet, "data-aspect":key, "key":window.T13NE.KeyGen("cardaspect") })));
					}else{
						return null;
					}
				...
				//console.log("created facet aspect =",aspect);
				ret[index]=window.T13NE.CE("li",{"className":props.className+"-li","key":window.T13NE.KeyGen("list-item"+index)},
			...
			if (fel!=="any"&&fel!=="any "&&fel!=="all"&&fel!=="all "){
				...
				if (facet!==null){
					...
					ret[index] = window.T13NE.CE("details",{className:this.props.className+"-facet-details t13ne-facet-short", "data-facet":facet},
						...
						window.T13NE.CE(window.T13NE.FacetDisplay,{className:this.props.className+"-facet-display", "data-facets":fel, "data-facet":window.T13NE.Facets[facet].FacetName, "data-mode":"short"}, null)
					);
				}
			}
		},this);
