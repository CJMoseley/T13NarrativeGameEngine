...
class LoaderManager {
    ...
    constructor(viewManager) {
        ...
        this.tasks = [
            ...
            // Visual Loading Sequence - Now controlled by the Referee
            {
                name: 'Queue Visuals', action: async () => {
                    IntroSequence.hide();
                    const Referee = this.engine.getModule('Referee');
                    if (Referee && typeof Referee.playIntro === 'function') {
                        await Referee.playIntro();
                    } else {
                        Logger.warn("LoaderManager: Referee.playIntro not found, showing main menu.");
