...
export class Galaxy {
    ...
    async generateStars() {
        try {
            const response = await this.workerPool.execute('generateStars', {
                ...this.params,
                seed: this.params.seed || 'wormhole-galaxy'
            });

            if (response.type === 'starsGenerated') {
                this.stars = response.stars;
                Logger.message(`Galaxy: Worker generated ${this.stars.length} stars.`);
            }
            return this.stars;
        } catch (error) {
            Logger.error("Galaxy: Star generation worker failed, falling back to main thread.", error);
