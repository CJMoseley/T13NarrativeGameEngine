...
export class VegetationGenerator {
    ...
    generateTrees(count, densityMap, terrainGenerator) {
        ...
        for (let i = 0; i < count; i++) {
            const x = this._getSeededRandom([...this.planetData.seeds, i, 0.1]) * 500 - 250;
            const z = this._getSeededRandom([...this.planetData.seeds, i, 0.2]) * 500 - 250;
    ...
    generateTree(x, z, terrainGenerator) {
        ...
        const height = this._getSeededRandom([...this.planetData.seeds, x, z, 0.3]) * 10 + 5;
    ...
    generateBranches(position, height, depth) {
        ...
        const branches = [];
        const numBranches = Math.floor(this._getSeededRandom([...this.planetData.seeds, position.x, position.y, position.z, 0.4]) * 3) + 2;

        for (let i = 0; i < numBranches; i++) {
            const newHeight = height * (this._getSeededRandom([...this.planetData.seeds, position.x, i, 0.5]) * 0.4 + 0.4);
            const newPosition = {
                x: position.x + (this._getSeededRandom([...this.planetData.seeds, i, 0.6]) - 0.5) * height,
                y: position.y + height * 0.5,
                z: position.z + (this._getSeededRandom([...this.planetData.seeds, i, 0.7]) - 0.5) * height,
            };
            branches.push({
