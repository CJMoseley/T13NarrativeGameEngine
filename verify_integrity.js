...
try {
    ...
    try {
        ...
        if (match) {
            ...
            console.error(`Context at ${pos}: ...${cardsData.substring(pos - 20, pos + 20)}...`);
        }
        process.exit(1);
    }
    ...
    cards.forEach((card, index) => {
        try {
            ...
            // Check for Joker specific if needed, but assuming Suit-Card pattern holds based on user request.
            if (card.Card.includes("Joker")) {
                // Adjust for known joker filenames if needed
                // e.g. 0-black-joker.json
                // If card.Suit is "0" and card.Card is "Black Joker", filename becomes "0-black-joker.json" -> matches.
            }
